...
export const auditSchema = z.object({
  ...
  website: z.union([z.literal(""), z.string().trim().url().startsWith("https://").max(2048)]),
  challenge: z.string().trim().min(1).max(5000),
  service: z.enum(["", "local-seo", "web-development", "technical-seo", "saas-development", "ai-integration", "both", "not-sure"]),
  honeypot: z.string().max(0).optional(),
});
